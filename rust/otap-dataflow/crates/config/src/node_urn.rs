...
mod tests {
    ...
    fn rejects_mismatches_and_invalids() {
        ...
        // Processor chain validated as wrong kind
        assert!(
            validate_plugin_urn("urn:otel:processor_chain:composite", NodeKind::Processor).is_err()
        );
        assert!(validate_plugin_urn("urn:otel:processor:debug", NodeKind::ProcessorChain).is_err());
