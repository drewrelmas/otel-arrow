    ...
    fn try_from(value: OtlpProtoMessage) -> Result<Self, Self::Error> {
        ...
        Ok(match value {
            OtlpProtoMessage::Logs(logs_data) => {
                ...
                OtapPdata::new_todo_context(OtlpProtoBytes::ExportLogsRequest(bytes.freeze()).into())
            }
            ...
            OtlpProtoMessage::Traces(trace_data) => {
                ...
                OtapPdata::new_todo_context(OtlpProtoBytes::ExportTracesRequest(bytes.freeze()).into())
            }
        })
    }
}
