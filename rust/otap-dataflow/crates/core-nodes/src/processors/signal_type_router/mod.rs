...
impl SignalTypeRouterMetrics {
    const fn inc_received(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_received_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => self.signals_received_metrics.inc(),
            otel_arrow_dfe_config::SignalType::Traces => self.signals_received_traces.inc(),
        }
    }
    const fn inc_routed_named(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_routed_named_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => self.signals_routed_named_metrics.inc(),
            otel_arrow_dfe_config::SignalType::Traces => self.signals_routed_named_traces.inc(),
        }
    }
    const fn inc_routed_default(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_routed_default_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => self.signals_routed_default_metrics.inc(),
            otel_arrow_dfe_config::SignalType::Traces => self.signals_routed_default_traces.inc(),
        }
    }
    const fn inc_nacked(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_nacked_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => self.signals_nacked_metrics.inc(),
            otel_arrow_dfe_config::SignalType::Traces => self.signals_nacked_traces.inc(),
        }
    }
    const fn inc_rejected_route_full(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_rejected_route_full_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => {
                self.signals_rejected_route_full_metrics.inc()
            }
            otel_arrow_dfe_config::SignalType::Traces => {
                self.signals_rejected_route_full_traces.inc()
            }
        }
    }
    const fn inc_rejected_route_closed(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => {
                self.signals_rejected_route_closed_logs.inc()
            }
            otel_arrow_dfe_config::SignalType::Metrics => {
                self.signals_rejected_route_closed_metrics.inc()
            }
            otel_arrow_dfe_config::SignalType::Traces => {
                self.signals_rejected_route_closed_traces.inc()
            }
        }
    }
    const fn inc_dropped(&mut self, st: otel_arrow_dfe_config::SignalType) {
        match st {
            otel_arrow_dfe_config::SignalType::Logs => self.signals_dropped_logs.inc(),
            otel_arrow_dfe_config::SignalType::Metrics => self.signals_dropped_metrics.inc(),
            otel_arrow_dfe_config::SignalType::Traces => self.signals_dropped_traces.inc(),
        }
    }
}
