            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    otel_arrow_dfe_telemetry::otel_debug!(
                        "kafka.exporter.producer.poll_thread_started"
                    );
                    loop {
...
{
    fn drop(&mut self) {
        otel_arrow_dfe_telemetry::otel_debug!("kafka.exporter.producer.destroying");
        if let Some(handle) = self.handle.take().and_then(Arc::into_inner) {
            otel_arrow_dfe_telemetry::otel_debug!("kafka.exporter.producer.stopping_poll");
            self.should_stop.store(true, Ordering::Relaxed);
            otel_arrow_dfe_telemetry::otel_debug!("kafka.exporter.producer.awaiting_poll_thread");
            match handle.join() {
                Ok(()) => {
                    otel_arrow_dfe_telemetry::otel_debug!("kafka.exporter.producer.poll_stopped")
                }
                Err(e) => otel_arrow_dfe_telemetry::otel_warn!(
                    "kafka.exporter.producer.poll_thread_join_failed",
                    error = ?e,
                ),
            };
        }
        otel_arrow_dfe_telemetry::otel_debug!("kafka.exporter.producer.destroyed");
    }
}
