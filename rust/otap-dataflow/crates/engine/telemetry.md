...
| `node.producer.produced.log_records` | Log records produced (emitted) by the node, per signal. For receivers this is the `receiver_accepted` count. | `crates/engine/src/channel_metrics.rs` |
