...
/// User-facing configuration for a processor chain node.
///
...
pub struct ProcessorChainConfig {
    /// Ordered list of sub-processor definitions.
...
pub struct SubProcessorConfig {
    ...
    /// in telemetry (e.g. `chain/my_filter`). If omitted, the index is
    /// used (e.g. `chain/0`).
    #[serde(default)]
    pub name: Option<String>,
    /// The processor type URN (e.g. `processor:attribute`).
    pub r#type: String,
    /// Optional sub-processor-specific configuration.
    #[serde(default)]
