...
/// 3. Feeds each output to the second sub-processor, collecting its outputs
...
impl<PData> ProcessorChainNode<PData> {
    ...
    pub fn new(
        ...
        let sub_processors = sub_processors
            .into_iter()
            .map(|(processor, _node_id, _metrics_reporter)| SubProcessor { processor })
            .collect();
    ...
    async fn process(
        ...
        match msg {
            ...
            Message::PData(pdata) => {
                ...
                // Use pre-allocated staging buffers, swapping between them.
                // `current` holds the inputs for the current stage.
...
mod tests {
    ...
    /// Create a real EffectHandler wired to an mpsc channel so we can
    /// inspect what the chain sends downstream.
